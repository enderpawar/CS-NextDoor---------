...
| **CV 모듈 2** (히스토그램 분석) | ✅ 완료 — `src/lib/cv/changeDetection.ts` 4 메트릭 이식 + **실측 영상 ablation 반영**(2026-05-22). synthetic CORREL+GRAY+w=5+threshold=0.9999 → 실측 CORREL+RGB+w=3+threshold=0.9995 (F1=0.34, AUC=0.70). 데이터: `data/live-frames/real/Test_video.mp4` (54s, 1080p, 6 transitions). 스크립트: `notebooks/run_module2_real_ablation.py` |
